...
- Type checks: `typescript` + `@astrojs/check` (`npm run check`, `checkJs: true`)
...
- `src/components/layout/FloatingIsland.astro`
  - Appears after scroll, includes language controls and settings cog
- `src/components/i18n/LanguageSelector.astro`
...
## Key UI Systems
- Translation/language slots: `src/components/i18n/T.astro`
- Theme: root `.dark` class + persisted setting
- Study setting:
  - `showDefinitionHighlights` controls `src/components/content/Explain.astro` visuals/tooltip behavior
- Island setting:
...
- Changes to global preferences usually touch:
  - `public/scripts/settings-state.js`
  - `src/components/settings/SettingsCog.astro` (value toggles/labels only)
  - `public/scripts/settings-panel-controller.js` (sheet open/close behavior)
  - `src/components/layout/BaseHead.astro` (script loading/pre-paint behavior)
- Run `npm run check` after JS/TS changes.
